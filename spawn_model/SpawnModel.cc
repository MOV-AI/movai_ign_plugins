...
{
  ignmsg << "SpawnModel::Configure" << std::endl;
                        ...
                        ignition::gazebo::EntityComponentManager &_ecm)
{
  ...
  if (this->string_ready && this->spawnObject)
  {
    ...
    for (int i = 0; i < this->objectPosition.size(); i++)
    {
      ...
      ignmsg << "Transport success: " << result << std::endl;
      ignmsg << "Create service returned: " << res.data() << std::endl;
...
{
  ignmsg << "Received " << _msg.pose_size() << " poses" << std::endl;
...
{
  ignmsg << "Received model array: " << _msg.data() << std::endl;
  // Check if the received data is not empty
  if (!_msg.data().empty())
  {
